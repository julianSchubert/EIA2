...
function Sortiertekartenanzeigen():void{//Hier fehlt noch die Funktion die die neuen Handkarten zurückgibt
  //  Kartensortieren();
}
...
function Karteziehen():void{
    ...
    let prodElement = document.createElement('div');
    let aufderhand: string = `<p class="${Karten[neuezahl].typ}">${Karten[neuezahl].typ} ${Karten[neuezahl].wert}</p>`
    prodElement.innerHTML = aufderhand;
    document.getElementById("Handkarten").appendChild(prodElement);
